...
load_dotenv(os.path.expanduser("~/.openclaw/skills/team-tasks/scripts/.env"), override=True)
...
def load_task_template(template_name: str, **kwargs) -> str:
    """
    Load task template from templates/ directory.
    Supports variable substitution via **kwargs.
    
    Example:
        load_task_template("phase1-analyst-analyze-requirements.md", goal="fix bug", work_dir="/repo")
    """
    template_path = os.path.join(_TEMPLATE_DIR, template_name)
    if not os.path.exists(template_path):
        print(f"⚠️  模板不存在: {template_path}，使用内联模板", file=sys.stderr)
        return None
...
def cmd_phase1(args):
    ...
    task_analyze = load_task_template("phase1-analyst-analyze-requirements.md", 
        goal=goal, work_dir=work_dir, docs_subdir=docs_subdir)
    data["stages"]["analyze-requirements"] = make_stage(
        agent_id="analyst",
        task=task_analyze or f"""需求分析：{goal}
...
""",
    ...
    task_prd = load_task_template("phase1-pm-create-prd.md", 
        work_dir=work_dir, docs_subdir=docs_subdir)
    data["stages"]["create-prd"] = make_stage(
        agent_id="pm",
        task=task_prd or f"""PRD 细化：Epic/Story 拆分、验收标准、优先级矩阵
...
""",
    ...
    task_arch = load_task_template("phase1-architect-design-architecture.md",
        work_dir=work_dir, docs_subdir=docs_subdir)
    data["stages"]["design-architecture"] = make_stage(
        agent_id="architect",
        task=task_arch or f"""系统架构设计
...
""",
    ...
    task_decision = load_task_template("phase1-coord-decision.md", work_dir=work_dir)
    data["stages"]["coord-decision"] = make_stage(
        agent_id="coord",
        task=task_decision or f"""决策：读取产物，决定是否开启阶段二开发
...
""",
    ...
    # ★ 自动通知（默认开启）：把根任务标记为 ready，触发 wake_downstream 发通知
    import subprocess as _sub
    _notify_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "slack_notify_templates.py")
    _root_tasks = [tid for tid, t in data["stages"].items() if not t.get("dependsOn")]
    for _tid in _root_tasks:
        _old = data["stages"][_tid]["status"]
        data["stages"][_tid]["status"] = "ready"
        data["stages"][_tid]["updatedBy"] = "cli"
        if "logs" not in data["stages"][_tid]:
            data["stages"][_tid]["logs"] = []
        data["stages"][_tid]["logs"].append({
            "time": now_iso(),
            "event": f"status: {_old} → ready (phase1 初始化)",
        })
        _log_update(None, _tid, _old, "ready", "cli")
        # 触发 wake_downstream 通知下游
        wake_downstream(data, _tid)
    save_project(project, data)
    # 发送 Slack 通知
    for _tid in _root_tasks:
        _agent = data["stages"][_tid].get("agent", "")
        if _agent not in _AGENT_CHANNEL_MAP:
            continue
        _task_desc = data["stages"][_tid].get("task", _tid)
        _constraints = "; ".join(data["stages"][_tid].get("constraints", []))
        _output = data["stages"][_tid].get("output", "")
        try:
            _sub.run([
                "python3", _notify_script,
                "notify-ready", _agent,
                f"{project}/{_tid}",
                _task_desc, _constraints, _output
            ], env=os.environ.copy(), capture_output=True, text=True, timeout=10)
            print(f"📤 已自动发送 {_tid} 通知到 #{_agent}")
        except Exception as _e:
            print(f"⚠️ 自动通知失败: {_e}", file=sys.stderr)
...
def cmd_phase2(args):
    ...
    for epic in epics:
        ...
        task_dev = load_task_template("phase2-dev.md",
            epic=epic, work_dir=work_dir, impl_plan=impl_plan, agents_file=agents_file)
        data["stages"][f"dev-{epic_id}"] = make_stage(
            agent_id="dev",
            task=task_dev or f"""开发 Epic: {epic}
...
""",
        ...
        task_tester = load_task_template("phase2-tester.md",
            epic=epic, work_dir=work_dir, agents_file=agents_file)
        data["stages"][f"tester-{epic_id}"] = make_stage(
            agent_id="tester",
            task=task_tester or f"""测试 Epic: {epic}
...
""",
        ...
        task_reviewer = load_task_template("phase2-reviewer.md", epic=epic, work_dir=work_dir)
        data["stages"][f"reviewer-{epic_id}"] = make_stage(
            agent_id="reviewer",
            task=task_reviewer or f"""审查 Epic: {epic}（第一步：功能审查）
...
""",
        ...
        task_reviewer_push = load_task_template("phase2-reviewer-push.md", epic=epic, work_dir=work_dir)
        data["stages"][f"reviewer-push-{epic_id}"] = make_stage(
            agent_id="reviewer",
            task=task_reviewer_push or f"""审查 Epic: {epic}（第二步：推送验证）
...
""",
    ...
    # Build epic table for template
    epic_table_lines = []
    for epic in epics:
        epic_table_lines.append(f"| {epic} | `git log --oneline` | `npm test` | `CHANGELOG.md` 更新 | `git fetch && git log origin/main` |")
    epic_table = "\n".join(epic_table_lines)

    task_completed = load_task_template("phase2-coord-completed.md",
        epic_count=len(epics), work_dir=work_dir, epic_table=epic_table)

    data["stages"]["coord-completed"] = make_stage(
        agent_id="coord",
        task=task_completed or f"""项目完成确认（{len(epics)} 个 Epic 收口）
...
""",
    ...
    # ★ 自动通知（默认开启）：把已满足依赖的任务标记为 ready，触发 wake_downstream 发通知
    import subprocess as _sub
    _notify_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "slack_notify_templates.py")
    # 找所有依赖已全部 done/skipped 的任务
    _ready_tasks = []
    for _tid, _t in data["stages"].items():
        if _t.get("status") not in ("pending", "rejected", "blocked"):
            continue
        _deps = _t.get("dependsOn", [])
        if not _deps:
            _all_done = True
        else:
            _all_done = all(
                data["stages"].get(_d, {}).get("status") in ("done", "skipped")
                for _d in _deps
            )
        if _all_done:
            _ready_tasks.append(_tid)
    for _tid in _ready_tasks:
        _old = data["stages"][_tid]["status"]
        data["stages"][_tid]["status"] = "ready"
        data["stages"][_tid]["updatedBy"] = "cli"
        if "logs" not in data["stages"][_tid]:
        ...
        data["stages"][_tid]["logs"].append({
            "time": now_iso(),
            "event": f"status: {_old} → ready (phase2 初始化)",
        })
        _log_update(None, _tid, _old, "ready", "cli")
        wake_downstream(data, _tid)
    save_project(project, data)
        ...
        if new_status == "done":
            ...
            for task_id, task in data["stages"].items():
                if task["status"] in ("done", "skipped"):
                    continue
                deps = task.get("dependsOn", [])
                if stage_id not in deps or task["status"] == "ready":
                    continue
                # 关键修复：只有当该任务的所有依赖都完成时才能唤醒（与 wake_downstream 一致）
                all_deps_done = all(
                    data["stages"].get(d, {}).get("status") in ("done", "skipped")
                    for d in deps
                )
...
def cmd_claim(args):
    ...
    # 输出任务详情
    report_lines = []
    report_lines.append(f"# 阶段任务报告：{stage_id}")
    report_lines.append(f"**项目**: {args.project}")
    report_lines.append(f"**领取 agent**: {claiming_agent}")
    report_lines.append(f"**领取时间**: {now_iso()}")
    report_lines.append(f"**版本**: rev {expected_rev} → {new_rev}")
    report_lines.append("")

    report_lines.append(f"## 项目目标")
    report_lines.append(f"{data.get('goal', '')}")
    report_lines.append("")

    report_lines.append(f"## 阶段任务")
    report_lines.append(f"{stage.get('task', '')}")
    report_lines.append("")

    if stage.get("constraints"):
        report_lines.append(f"## 🔴 约束清单")
        for c in stage["constraints"]:
            report_lines.append(f"- {c}")
        report_lines.append("")

    if stage.get("checklist"):
        report_lines.append(f"## ✅ 检查单")
        for c in stage["checklist"]:
            report_lines.append(f"- [ ] {c}")
        report_lines.append("")

    if stage.get("output"):
        report_lines.append(f"## 📦 产出路径")
        report_lines.append(f"{stage['output']}")
        report_lines.append("")

    if dep_outputs:
        report_lines.append(f"## 📤 上游产物")
        for dep_id, out in dep_outputs.items():
            report_lines.append(f"- {dep_id}: {out}")
        report_lines.append("")

    # 输出到 stdout
    print(f"✅ Claimed: {stage_id} (rev {expected_rev} → {new_rev})")
    print()
    for line in report_lines[1:]:  # 跳过标题行，只输出内容
        print(line)

    # 生成报告文件
    import os
    import time
    project_dir = os.path.join("/root/.openclaw/vibex/docs/", args.project)
    os.makedirs(project_dir, exist_ok=True)
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    report_file = os.path.join(project_dir, f"{claiming_agent}-{stage_id}-report-{timestamp}.md")
...
def main():
    ...
    p.add_argument("--notify", action="store_true", help="自动发送 Slack 通知（派发 analyze-requirements 给 analyst）")
